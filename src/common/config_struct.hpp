...
std::string const FILENAME = "rtsp_parameters.conf";
#define MAXBUF 2048
#define DELIM "="
...
struct config
{
    ...
    char rtsp_cert_pem_client[MAXBUF];
    char rtsp_cert_key_client[MAXBUF];
    ...
    if (file != NULL)
    {
        ...
        while(fgets(line, sizeof(line), file) != NULL)
        {
            ...
            if (i == 0) {
                ...
                memcpy(rtsp_config.rtsp_cert_pem_client,cfline,strlen(cfline));
            } else if (i == 4) {
                memcpy(rtsp_config.rtsp_cert_key_client,cfline,strlen(cfline));
            } else if (i == 5) {
