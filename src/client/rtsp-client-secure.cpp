...
int main (int argc, char *argv[])
{
    ...
    //Video Sink
    video_sink = gst_element_factory_make ("fakesink", "autovideosink0");
    g_assert (video_sink);
    //video
    ...
    */
    ...
    GError *error=NULL;
    cert = g_tls_certificate_new_from_files(rtsp_config.rtsp_cert_pem_client,rtsp_config.rtsp_cert_key_client,&error);
    ...
    GTlsDatabase* database = g_tls_file_database_new (rtsp_config.rtsp_cert_pem_client, &error);
    ...
    ca_cert = g_tls_certificate_new_from_file(rtsp_config.rtsp_cert_pem_client,&error);
